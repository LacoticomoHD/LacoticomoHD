...
import { AUTO_HIDE_REPORTS, fetchAllReports, fetchHiddenShopIds, setReportStatus } from '@/lib/api';
...
export function ReportsInboxScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      fetchHiddenShopIds().then(setHiddenShops).catch(() => {});
    }, [])
  );
  ...
  const toggleStatus = async (report: ReportWithShop) => {
    ...
    try {
      ...
      setHiddenShops(await fetchHiddenShopIds());
    } catch (e) {
